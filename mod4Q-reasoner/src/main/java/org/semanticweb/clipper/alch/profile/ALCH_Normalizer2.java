...
public class ALCH_Normalizer2 implements OWLAxiomVisitorEx<Object> {
    ...
    /*previously used only to check if an ontology is in ALCH profile*/
    //private ALCH_Sub_ClassExpressionChecker sub;

    public ALCH_Normalizer2() {
    }
    ...
    public OWLOntology normalize(OWLOntology ontology) {
        ...
        //run first normalization procedure
    ...
    public OWLOntology normalizePhaze1(OWLOntology ontology) {
        ...
        for (OWLAxiom axiom : ontology.getAxioms()) {
            axiom.getNNF().accept(this);                    //delay the NNF
        }
    ...
    public Object visit(OWLSubClassOfAxiom axiom) {
        ...
        // TODO:first we check if an expression is in ALCH Profile, if not we issue a warning, add this also to Disjoint Classes Axiom and Equivalent Classes Axiom
        ...
        // A' subclass C
            ...
            if (subClass.getClassExpressionType() == ClassExpressionType.OBJECT_INTERSECTION_OF) {
                ...
                Set<OWLClassExpression> foralls = new HashSet<OWLClassExpression>();
                Set<OWLClassExpression> negations = new HashSet<OWLClassExpression>();
                ...
                for (OWLClassExpression op : operands) {
                    if ((op.getClassExpressionType() == ClassExpressionType.OBJECT_ALL_VALUES_FROM)) {
                        foralls.add(op);
                        forallFound=true;
                    }
                }
                ...
                if (normalized) {
                    ...
                    newDis.add(superClass);
                    for (OWLClassExpression op : operands) {
                        if (op.getClassExpressionType()== ClassExpressionType.OBJECT_ALL_VALUES_FROM) {
                            OWLObjectAllValuesFrom all = (OWLObjectAllValuesFrom) op;
                            OWLObjectSomeValuesFrom some = factory.getOWLObjectSomeValuesFrom(all.getProperty(), factory.getOWLObjectComplementOf(all.getFiller()));
                            newDis.add(some);
                        }else if(op.getClassExpressionType()== ClassExpressionType.OBJECT_COMPLEMENT_OF){
                            OWLObjectComplementOf neg  = (OWLObjectComplementOf)op;
                            newDis.add(neg.getOperand());
                        }else if(!(op.getClassExpressionType() == ClassExpressionType.OWL_CLASS)) {
                            OWLClass freshClass = getFreshClass();
                    ...
                    //case where there were all of conjuncts were negations or forall, which were pushed on the right
                    if(newOps.size()==0 && newDis.size()>1){
                        factory.getOWLSubClassOfAxiom(factory.getOWLThing(),
                                                      factory.getOWLObjectUnionOf(newDis))
                                .accept(this);

                    }//case where there were negations or forall pushed on the right, and there is only one conjunct left on lhs
                    else if (newOps.size()==1 && newDis.size()>1) {
                        List<OWLClassExpression> list = new ArrayList<OWLClassExpression>(newOps);

                        factory.getOWLSubClassOfAxiom(list.get(0),
                                                      factory.getOWLObjectUnionOf(newDis))
                                .accept(this);
                    }//case where there were negations or forall pushed on the right, but still there are more than one conjuncts on the left
                    else if(newOps.size()>1 && newDis.size()>1){
                        factory.getOWLSubClassOfAxiom(factory.getOWLObjectIntersectionOf(newOps),
                                                      factory.getOWLObjectUnionOf(newDis))
                                .accept(this);
                    }//case where there are no negations and forall on the left
                    else if(newOps.size()>1 && newDis.size()==1){
                        factory.getOWLSubClassOfAxiom(factory.getOWLObjectIntersectionOf(newOps),
                                                      superClass)
                                .accept(this);
                    }//all cases should have been taken care of with the above steps
                    else{
                        System.err.println(axiom.toString());
                    }
                }
            }
            /*OK*/
            ...
            // all(R,A') subclass B
                ...
                factory.getOWLSubClassOfAxiom(
                        factory.getOWLObjectIntersectionOf(fresh2,all.getFiller()),
                        factory.getOWLNothing()).accept(this);
            }
